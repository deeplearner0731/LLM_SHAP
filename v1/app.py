...
def compare_generators(
    *, explain_method, explain_scope, outcome_type, loss_name, model_type,
    ...
    for rep in range(int(n_reps)):
        ...
        tok_in += int(u.get("prompt_tokens") or 0)
        tok_out += int(u.get("completion_tokens") or 0)
        temp_applied_any = temp_applied_any or bool(meta.get("temperature_applied"))
    ...
    return {
        ...
        "temperature_applied": bool(temp_applied_any),
...
def adaptation_analysis(
    *, explain_method, explain_scope, outcome_type, loss_name, model_type,
    ...
    # Tracked because the whole experiment rests on within-audience similarity being a real
    # noise floor. If the endpoint silently drops the temperature the LLM may be near
    # deterministic, the floor collapses to ~1.000, and the adaptation gap looks large for a
    # reason that has nothing to do with adaptation.
    temp_applied_any, temp_notes = False, set()
    for label, ptext in prompts:
        ...
        for rep in range(int(n_reps)):
            ...
            temp_applied_any = temp_applied_any or bool(meta.get("temperature_applied"))
            if meta.get("temperature_note"):
                temp_notes.add(meta["temperature_note"])
            g.append(text)
        l_groups.append(g)
    ...
    return {
        ...
        "llm_model": llm_model, "temperature": temperature,
        "temperature_applied": bool(temp_applied_any),
        "temperature_notes": sorted(temp_notes),
        "tokens": {"prompt": tok_in, "completion": tok_out} if (tok_in or tok_out) else {},
        "endpoint": base_url or "hosted API (api.openai.com)",
        "local_endpoint": bool(local_endpoint),
    }
...
def _temperature_line(meta: Dict[str, Any]) -> str:
    """What temperature was requested, and whether the endpoint honoured it.

    Reported for every generation rather than only in the sweep: a number copied into a
    manuscript table needs the setting that produced it, and "requested but rejected" is a
    materially different provenance from "requested and applied".
    """
    req = meta.get("temperature")
    if req is None:
        return ("Temperature: not set (parameter omitted; the endpoint's own default "
                "applies, which is 1.0 on the hosted API)")
    if meta.get("temperature_applied"):
        return f"Temperature: {float(req):.2f} (accepted by the endpoint)"
    note = meta.get("temperature_note") or "rejected by this model"
    return (f"Temperature: {float(req):.2f} REQUESTED BUT NOT APPLIED -- {note}. "
            "Generations used the server default; do not report them as this setting.")
...
def format_adaptation(res: Optional[Dict[str, Any]]) -> list:
    ...
    L = ["=== Capability the metrics above cannot see: adaptation to the request ===",
         ...
         ""]
    if res.get("temperature") is not None and float(res["temperature"]) == 0.0:
        L.append(
            "NOTE: at temperature 0 the LLM is close to deterministic, so within-audience "
            "similarity approaches 1.000 and the noise floor this experiment controls "
            "against largely disappears. A positive gap measured here is weak evidence of "
            "adaptation. Use a non-zero temperature (0.3 is reasonable) for this mode.")
        L.append("")
...
app_ui = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.sidebar(
            ...
            ui.input_text("llm_temperature", "Temperature", value="0",
                          placeholder="blank = endpoint default"),
            ui.help_text(
                "Sampling temperature for the narrative, the follow-up chat and the two "
                "generator comparisons in the Sensitivity panel. 0 is the reproducible "
                "setting and the default here, because these are structured "
                "attribution-reporting tasks. Leave blank to omit the parameter entirely "
                "and take whatever the endpoint defaults to -- for the hosted API that is "
                "1.0, and for a local server it depends on the server. The temperature "
                "sweep in the Sensitivity panel sets its own values and ignores this field."
            ),
                ...
                ui.accordion_panel(
                    ...
                    ui.panel_conditional(
                        ...
                        ui.help_text(
                            ...
                            "the Discussion tab, measure. Both modes use the sidebar "
                            "Temperature. Set it to 0 for 'Template baseline vs LLM', which "
                            "compares reproducibility and so should give the LLM its best "
                            "setting; use a non-zero value (0.3) for 'Audience adaptation', "
                            "which needs within-audience variation as its noise control."
                        ),
                    ...
                    ui.panel_conditional(
                        ...
                        ui.help_text(
                            ...
                            "from 'Endpoint and privacy' above. This sweep sets each "
                            "temperature itself and ignores the Temperature field in the "
                            "sidebar."
                        ),
                    ),
...
def server(input, output, session):
    ...
    def _llm_temp():
        """The requested sampling temperature, or None to omit the parameter.

        Blank is not the same as 0: None omits `temperature` from the request body, so the
        endpoint's own default applies (1.0 on the hosted API). Returning 0.0 for a blank
        field would silently change what the user asked for, and returning None for a
        requested 0 would silently make a reproducibility claim untrue, so the two are kept
        distinct all the way down to `_call_chat`. Bad input is refused rather than
        defaulted, because a temperature that quietly reverts is exactly the failure the
        sweep's `temperature_applied` flag exists to catch.
        """
        raw = str(input.llm_temperature() or "").strip()
        if not raw:
            return None
        try:
            t = float(raw)
            ...
            raise ValueError(
                f"Temperature must be a number or blank; got {raw!r}. Blank omits the "
                "parameter and uses the endpoint default.")
        if not np.isfinite(t) or not (0.0 <= t <= 2.0):
            raise ValueError(
                f"Temperature must be between 0 and 2; got {t}. Blank omits the parameter "
                "and uses the endpoint default.")
        return t
    ...
    def llm_out():
        ...
        try:
            ...
            head.append(_temperature_line(meta))
    ...
    def _export_config():
        ...
        return {
            ...
            "llm": {"endpoint": g("llm_base_url"), "model": g("llm_model"),
                    ...
                    # The string, not the parsed value: blank is a meaningful setting
                    # ("parameter omitted") and must not be recorded as 0.
                    "temperature": (str(g("llm_temperature") or "").strip()
                                    or "not set (endpoint default)")},
        }
