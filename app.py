...
def fit_model(
    ...
    if model_type == "Linear Regression":
        ...
        if loss_name == "Original (clinical)":
            ...
            model = ModifiedLinearRegressor(loss_name=loss_name, reg_lambda=1e-6, maxiter=500)
            ...
            if sigpos_exists:
                metrics_out["train_auc_sigpos"] = _safe_auc(sigpos_train, pred_tr)
                if HAS_TEST:
                    metrics_out["test_auc_sigpos"] = _safe_auc(sigpos_test, pred_te)
    ...
    if model_type == "Logistic Regression":
        ...
        if loss_name == "Original (clinical)":
            ...
            model = ModifiedLogisticRegressor(loss_name=loss_name, reg_lambda=1e-6, maxiter=500)
    ...
    if model_type == "Cox Regression":
        ...
        if loss_name == "Original (clinical)":
            ...
            model = ModifiedCoxRegressor(loss_name=loss_name, reg_lambda=1e-6, maxiter=500)
...
def save_shap_plots(
    ...
    plt.title(f"SHAP decision plot: patient {i}")
    ...
    def llm_out():
        ...
        if explain_method == "SHAP":
            if "explainer" not in st or "shap_values" not in st:
                return "No SHAP artifacts available. Please run analysis again."
